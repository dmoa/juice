...
static uint16_t GetU16(char* memory) {
	uint8_t* p = (uint8_t*)(memory);
	return (((uint16_t)p[1]) << 8) |
		   (((uint16_t)p[0]));
}

static uint32_t GetU32(void* memory) {
	uint8_t* p = (uint8_t*)(memory);
...
static Ase_Output AseLoad(std::string file_path) {
    ...
    if (file) {
        ...
        Ase_Output output = {pixel_data, header.width, header.height, palette, tags, frame_durations, header.num_frames};
        return output;
